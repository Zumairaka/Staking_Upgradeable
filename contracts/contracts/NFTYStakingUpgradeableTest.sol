...
contract NFTYStakingUpgradeableTest is
    ...
    ReentrancyGuardUpgradeable
{
    ...
    // variable added as part of upgrading
    uint256[2] _newRewardRate;
    ...
    // function for returning the newly added reward rates (added as upgrade)
    function getNewRewardRates() external view returns (uint256[2] memory) {
        return _newRewardRate;
    }
    ...
    // function for retrieiving the staker data (added as upgrade)
    function getStakerData(address staker)
        external
        view
        isRealAddress(staker)
        returns (
            uint256 amount,
            uint256 stakingTime,
            uint256 reward,
    ...
    {
        StakeData storage stakeData = StakersData[staker];
        amount = stakeData.amount;
        require(amount > 0, "NFTYStaking: address is not a staker");

        stakingTime = stakeData.stakingTime;
        reward = stakeData.reward;
        lastClaimTime = stakeData.lastClaimTime;
    }

    // function for adding more rank (added as upgrade)
    function addRank(uint8 rank, uint256 rate) external onlyRole(ADMIN_ROLE) {
        require(rate > 0 && rate <= 100000, "NFTYStaking: invalid rate");

        require(rank < 2, "NFTYStaking: invalid rank");

        _newRewardRate[rank] = rate;
    }
        ...
        returns (uint256 rewardRate)
    {
        ...
        if (amount >= _getAmount(1) && amount < _getAmount(500)) {
            ...
            amount >= _getAmount(100000) && amount < _getAmount(150000)
        ) {
            rewardRate1 = _rewardRate[5];
        } else {
            rewardRate1 = _newRewardRate[0];
        }
        ...
        } else if (stakingTime >= 365 days && stakingTime < 450 days) {
            rewardRate2 = _rewardRate[5];
        } else {
            rewardRate2 = _newRewardRate[0];
        }
